...
trap(struct Trapframe *tf)
{
	...
	if (!curenv) {
		cprintf("curenv is lost!\n");
	} else if (curenv->env_status != ENV_RUNNING) {
		cprintf("curenv status is not right!\n");
		cprintf("%d\n", curenv->env_status);
	} else {
		cprintf("should not be there!\n");
	}
 	assert(curenv && curenv->env_status == ENV_RUNNING);
	env_run(curenv);
}

static int trap_from_exception(uintptr_t esp)
{
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if (trap_env->env_pgfault_upcall) {
		...
		if (current_exception_stack_size > sizeof(struct UTrapframe)) {
			// size of current exception stack is larger than the size of struct UTrapframe
			// which means we have enough room to push a struct UTrapframe as an argument.
			...
			struct UTrapframe *pf_upcall_arg = (struct UTrapframe *)(trap_env->env_tf.tf_esp);
