...
mem_init(void)
{
	...
	// Swit-ch from the minimal entry page directory to the full kern_pgdir
	// page- table we just created.	Our instruction pointer should be
	// some-where between KERNBASE and KERNBASE+4MB right now, which is
	// mapp-ed the same way by both page tables.
	//-
	// If t-he machine reboots at this point, you've probably set up your
	// kern_pgdir wrong.
...
mem_init_mp(void)
{
	...
	for (int i = 0; i < NCPU; i++) {
		...
		unsigned char *kstackaddr_i = percpu_kstacks[i];
...
page_init(void)
{
	...
	// free pages!
	int IORegion_begin = ROUNDDOWN(IOPHYSMEM, PGSIZE) / PGSIZE;
	int IORegion_end = ROUNDUP(EXTPHYSMEM, PGSIZE) / PGSIZE;
	int KernRegion_addr = PADDR(boot_alloc(0));
	int KernRegion_end = ROUNDUP(KernRegion_addr, PGSIZE) / PGSIZE;
	int mpentry_page = MPENTRY_PADDR / PGSIZE;
...
page_alloc(int alloc_flags)
{
	...
	struct PageInfo *free_page = NULL;
	if (page_free_list) {
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (pgdir[pgdir_index] & PTE_P) {
		...
		pte_t *va_pgtable = (pte_t*)KADDR(phys_pgtable);
		...
		if (create) {
			...
			if (pgtable_pg) {
				...
				pgtable_pg->pp_ref++;
				pte_t *va_pgtable = page2kva(pgtable_pg);
				int pgtable_index = PTX(va);
				ret_pte = va_pgtable + pgtable_index;
			} else {
				ret_pte = NULL;
...
//
bool pte_perm_check(const pte_t *test, int perm)
{
	if (!test) {
		return false;
	}
